...
### key作用原理(Vue里虚拟DOM的diff算法)
- 虚拟新旧DOM进行对比依赖的是:key的值
- 虚拟DOM的是存储在内存里的，真实DOM是根据虚拟DOM转换而来的
- 复用：虚拟DOM进行对比以后如果对比的结果是新旧虚拟DOM是一样的，那么就直接用旧虚拟DOM的真实DOM
- 最好使用数据的唯一标识作为数据的:key值